...
const statusTitle = computed(() => build.value.status === 'success' ? 'Build successful' : build.value.status === 'warning' ? 'Build passed with warnings' : 'Build failed')
...
<template>
  <section class="compiler-card" :class="`compiler-${build.status}`" aria-label="Readiness Compiler">
    <div class="compiler-head"><div><span class="eyebrow">READINESS COMPILER</span><h3>{{ statusTitle }}</h3></div><strong>{{ build.score }}<small>/100</small></strong></div>
